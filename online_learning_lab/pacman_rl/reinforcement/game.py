...
class Game:
    ...
    def run(self):
        ...
        while not self.gameOver and (self.horizon < 0 or timestep < self.horizon):
            ...
            if timestep == 1:
                step_dict["wall_pos"] = []
                for wall in self.state.data.layout.wallCoords:
                    step_dict["wall_pos"].append(wall)
            state_dict = dict()
            state_dict["agents_config"] = []
            for ag in self.state.data.agentStates:
                config_dict = dict()
                config_dict["pos"] = ag.configuration.getPosition()
                config_dict["dir"] = ag.configuration.getDirection()
                config_dict["isPacman"] = ag.isPacman
                config_dict["scaredTimer"] = ag.scaredTimer
                state_dict["agents_config"].append(config_dict)
            state_dict["food_pos"] = []
            for food in self.state.data.layout.foodCoords:
                if self.state.data.food[food[0]][food[1]]:
                    state_dict["food_pos"].append(food)
            state_dict["capsule_pos"] = []
            for caps in self.state.data.capsules:
                state_dict["capsule_pos"].append(caps)
            step_dict["state"] = state_dict
            ...
            step_dict["score"] = self.state.data.score
